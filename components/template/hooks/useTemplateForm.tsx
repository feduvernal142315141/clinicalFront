import {RequestCreateTemplate, TemplateVariableRequest} from "@/lib/entity/template/template";
...
const replaceVariablesWithSamples = (text: string, variables: TemplateVariable[]) => {
    let result = text;
    variables.forEach((v) => {
        if (v.sampleContent) {
...
const validateAndExtractPlaceholders = (text: string): {
    valid: boolean;
    error: string;
    placeholders: string[]
} => {
    ...
    while ((match = validPlaceholderRegex.exec(text)) !== null) {
        ...
        validPositions.push({start: match.index, end: match.index + match[0].length});
    }
...
const useTemplateForm = () => {
    ...
    const [variablesMedia, setVariablesMedia] = React.useState<TemplateVariable[]>([
        {id: "1", placeholder: "{{1}}", sampleContent: ""},
    ]);
    ...
    const {control, handleSubmit, watch, formState: {errors, isSubmitting},} = useForm<TemplateFormData>({
    ...
    const handleUpdateBodyText = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        setVariablesText(prev => {
            const newVariables: TemplateVariable[] = uniquePlaceholderIds.map(id => {
    ...
    const onSubmit = useCallback(async (values: TemplateFormData) => {
        try {
